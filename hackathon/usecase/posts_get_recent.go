...
func (uc *PostGetRecentUseCase) Execute(ctx context.Context, limit, offset int) (*[]model.PostWithUserAndCounts, error) {
	posts, err := uc.PostRepo.FindAllWithCounts(ctx, uc.DB, limit, offset)
	if err != nil {
		return nil, err
	}
	return posts, nil
